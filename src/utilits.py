...
def filter_strings(string: str) -> str:
    """
    Принимает строку, удаляет из нее лишние символы, прописанные в списке symbols. Возвращает без них
    """
    symbols = ['\n', '\r', '</p>', '<p>', '</li>', '<li>',
               '<b>', '</b>', '<ul>', '<li>', '</li>', '<br />', '</ul>']
...
def filter_salary(salary):
    """ функция фильтрации по зарплате"""
...
def create_db(database_name, DB_PASS):
    """ создание базы данных"""
    connection = psycopg2.connect(host="localhost", database='northwind',
...
def create_tables(database_name, DB_PASS):
    """ создание таблиц в базе данных """
    connection = psycopg2.connect(host="localhost", database=database_name,
                                  user="postgres", password=DB_PASS)
...
def fillter_db(employers: list[dict], database_name, DB_PASS):
    connection = psycopg2.connect(host="localhost", database=database_name,
                                  user="postgres", password=DB_PASS)
